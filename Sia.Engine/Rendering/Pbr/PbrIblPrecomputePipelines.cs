...
public sealed unsafe class PbrIblPrecomputePipelines
{
    internal Entity PrefilterPipeline { get; }
    internal Entity PrefilterBindGroupLayout { get; }
    internal Entity BrdfLutPipeline { get; }

    private PbrIblPrecomputePipelines(Entity prefilterPipeline, Entity prefilterBindGroupLayout, Entity brdfLutPipeline)
    {
    ...
    public static PbrIblPrecomputePipelines Create(World world, Entity device)
    {
        var deviceHandle = device.GetWgpu<WGPUDevice>();

        var prefilterShader = world.OwnWgpu(Wgpu.CreateWgslShaderModule(
            deviceHandle, PbrShaderSource.LoadIblPrefilterSpecular(), "ibl_prefilter_specular"));
        ...
        var brdfLutShader = world.OwnWgpu(Wgpu.CreateWgslShaderModule(
            deviceHandle, PbrShaderSource.LoadIblBrdfLut(), "ibl_brdf_lut"));
        ...
        return new PbrIblPrecomputePipelines(prefilterPipeline, prefilterBindGroupLayout, brdfLutPipeline);
    }
