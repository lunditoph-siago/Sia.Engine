...
public static class PbrRenderGraphHooks
{
        ...
        in GpuFrame frame)
    {
        ...
        hooks.UseRenderGraphPass(registry, lutPass, "pbr-ibl-brdf-lut", lutState.Value.Declare);
        ...
        RenderGraphPassKey pass)
    {
        ...
        hooks.UseComputeRenderGraphPass(registry, pass, "pbr-cluster-culling", state.Value.Declare);
        hooks.UseWgpuRenderGraphPassHandler(registry, pass, state.Value.Render);
    }
        ...
        RenderGraphTextureKey depth)
    {
        ...
        hooks.UseRenderGraphPass(registry, pass, "pbr-depth-prepass", state.Value.Declare);
        hooks.UseWgpuRenderGraphPassHandler(registry, pass, state.Value.Render);
    }
    ...
    {
        var state = hooks.UseRef(() => new ForwardPbrState(color, depth));
        ...
        hooks.UseRenderGraphPass(registry, pass, "pbr-forward-opaque", state.Value.Declare);
        hooks.UseWgpuRenderGraphPassHandler(registry, pass, state.Value.Render);
    }
    ...
    private sealed class ClusterCullingState
    {
        ...
        public void Update(PbrRenderer renderer, in GpuFrame frame, ClusterGridConfig clusterConfig)
        {
            _renderer = renderer;
            _frame = frame;
    ...
    private sealed class DepthPrepassState(RenderGraphTextureKey depth)
    {
        ...
        public void Update(PbrRenderer renderer, in GpuFrame frame, Entity camera)
        {
            _renderer = renderer;
            _frame = frame;
            _camera = camera;
    ...
    private sealed class ForwardPbrState(RenderGraphTextureKey color, RenderGraphTextureKey depth)
    {
        ...
        public void Update(PbrRenderer renderer, in GpuFrame frame, WGPULoadOp colorLoadOp, bool colorCacheable)
        {
            _renderer = renderer;
            _frame = frame;
        ...
        public void Render(WgpuReactiveRenderGraphPassContext context)
        {
            ...
            _renderer!.EncodeForwardPbr(in _frame, renderPass);
        }
    }
    ...
    private sealed class ShadowLayerState(int layer)
    {
        ...
        public void Update(PbrRenderer renderer, in GpuFrame frame)
        {
            _renderer = renderer;
            _frame = frame;
        }
