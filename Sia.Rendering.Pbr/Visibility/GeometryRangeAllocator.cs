...
// Offsets are reused only between submitted frames on the owning WebGPU queue.
internal sealed class GeometryRangeAllocator
{
    internal readonly record struct Range(int Offset, int Length);
    private readonly List<Range> _free;
    public GeometryRangeAllocator(int offset, int length) => _free = length > 0 ? [new(offset, length)] : [];
