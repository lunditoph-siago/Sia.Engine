...
internal sealed class PbrGeometryPage
{
    ...
    internal readonly record struct Cluster(float X, float Y, float Z, uint VertexOffset,
        float4 Maximum, float4 Sphere, float4 Cone, uint4 Work);
    [StructLayout(LayoutKind.Sequential)]
    internal readonly record struct Triangle(uint x, uint y);
    ...
    private PbrGeometryPage(ReadOnlyMemory<byte> bytes, Counts size) { Bytes = bytes; Size = size; }

    public static PbrGeometryPage Decode(ReadOnlyMemory<byte> bytes)
    {
        ...
        var page = new PbrGeometryPage(bytes, size);
        ...
        foreach (var c in page.Clusters) {
            ...
            for (var i = 0; i < c.Work.y; i++) if (page.Indices[nextReference + i] >= size.Vertices) throw new InvalidDataException("Invalid vertex reference.");
            for (var i = 0; i < c.Work.z; i++) {
                var t = page.Triangles[nextTriangle + i];
                if (t.x != c.VertexOffset || t.y != page.Indices[referenceCount + nextTriangle + i]
    ...
    public static PbrGeometryPage Cook(MeshPatchAsset asset)
    {
