...
public static partial class PbrRenderGraphHooks
{
    internal static RenderGraphTextureKey UseAtmosphereComposite(ref RenderGraphBuildContext graph,
        PbrViewState view, PbrExtractedView extracted, PbrRenderFeatureOptions options, RenderGraphTextureKey source, in RenderFrameContext frame)
    {
        var state = graph.UseState(static () => new AtmosphereCompositeState());
        if (options.AtmosphereTarget == source || options.AtmosphereTarget == options.HdrTarget || options.AtmosphereTarget == frame.ColorTarget
        ...
        return state.Atmosphere is null ? source : options.AtmosphereTarget;
    }
