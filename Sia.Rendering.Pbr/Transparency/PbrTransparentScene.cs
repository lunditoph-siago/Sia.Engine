...
public sealed unsafe class PbrTransparentScene
{
    ...
    public PbrTransparentScene(in GpuFrame frame, PbrSceneAsset scene)
    {
        ...
        try {
            ...
            var meshes = new Dictionary<int, (Entity Vertices, Entity Indices, uint Count, Aabb Bounds)>();
            ...
            foreach (var instance in scene.Instances.Span) {
                ...
                if (!meshes.TryGetValue(instance.Geometry, out var mesh)) {
                    ...
                    mesh = (Upload<MeshVertex>(data.Vertices, WGPUBufferUsage.Vertex, RenderGraphBufferUsage.Vertex),
                        ...
                        (uint)data.Indices.Length, data.Bounds);
                ...
                draws.Add(new(mesh.Vertices, mesh.Indices, mesh.Count, Own(Group(device, materialLayout, bindings)),
                    BoundsTransform.Apply(mesh.Bounds, instance.Transform)));
            }
    ...
    internal sealed class View
    {
        ...
        // rebuilds only batches whose visible ordered draws or bindings change.
        ...
        private readonly TransparentDrawList _drawList;
        ...
        {
            Owner = owner;
            _drawList = new(owner._draws.Select(draw => draw.Bounds).ToArray());
            _recordedOrder = new int[owner._draws.Length];
        ...
        public int VisibleCount => _drawList.Count;
        public void Select(in Sia.Engine.Camera.CameraMatrices camera)
            => _orderChanged |= _drawList.Update(in camera);
        ...
        public void Render(WgpuHandle<WGPURenderPassEncoder> pass, PbrViewState lighting)
        {
            ...
            if (bindingsChanged) Array.Fill(_recordedCounts, -1);
            if (bindingsChanged || _orderChanged) {
                for (var i = 0; i < bundleCount; i++) {
                    var start = i * DrawsPerBundle;
                    var count = System.Math.Min(DrawsPerBundle, _drawList.Count - start);
                    var order = _drawList.Items.Slice(start, count);
                    var recorded = _recordedOrder.AsSpan(start, count);
                    if (!bindingsChanged && _recordedCounts[i] == count && order.SequenceEqual(recorded)) { continue; }
            ...
            fixed (nint* bundles = _orderedBundles) {
                WgpuUnsafe.wgpuRenderPassEncoderExecuteBundles((WGPURenderPassEncoder*)pass.DangerousGetHandle(),
                    (nuint)bundleCount, (WGPURenderBundle**)bundles);
            }
        }
    ...
    internal sealed record Draw(Entity Vertices, Entity Indices, uint Count, Entity Group, Aabb Bounds);
