...
fn classify(@builtin(global_invocation_id) thread: vec3<u32>,
    ...
    if (all(thread.xy < size)) {
        let id = textureLoad(visibility, vec2<i32>(thread.xy), 0).x;
        var material = 0u;
        if (id > 0u && id <= visibility_camera.size_counts.z) {
            material = u32(visibility_instances[visibility_work[id - 1u].y].material.z);
        }
        if (material < arrayLength(&dispatches) / 3u) {
