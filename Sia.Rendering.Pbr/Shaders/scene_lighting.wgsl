...
fn scene_lighting(world_position: vec3<f32>, normal: vec3<f32>, view_dir: vec3<f32>,
    base_color: vec3<f32>, metallic: f32, roughness: f32, emissive: vec3<f32>, occlusion: f32,
    pixel_position: vec2<f32>) -> vec3<f32> {
    var accumulated = vec3<f32>(0.0);

    let view_z = (cluster_config.view * vec4<f32>(world_position, 1.0)).z;
    ...
    for (var i = 0u; i < directional_count; i = i + 1u) {
        ...
        if (i == 0u && shadow_config.params.y != 0u) {
            ...
            visibility = shadow_sample_pcf(
                shadow_atlas, cascade, shadow_layers[cascade], world_position);
        }
    ...
    for (var i = 0u; i < count; i = i + 1u) {
        ...
        if (is_spot && shadow_layer >= 0) {
            visibility = shadow_sample_pcf(
                shadow_atlas, shadow_layer, shadow_layers[shadow_layer], world_position);
        }
    ...
    return accumulated + ambient * occlusion + emissive;
}
