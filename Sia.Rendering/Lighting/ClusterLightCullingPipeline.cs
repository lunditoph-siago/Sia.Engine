...
{
    public Entity ComputePipeline { get; }
    ...
    public static ClusterLightCullingPipeline Create(World world, Entity device)
    {
        ...
        var shaderModule = world.OwnWgpu(Wgpu.CreateWgslShaderModule(
            deviceHandle, LightingShaderSource.LoadClusterLightCulling(), "cluster_light_culling"));
    ...
    public Entity CreateBindGroup(in GpuFrame frame, ClusterGridBuffers clusters, LightGpuStore lights)
        => frame.ResourceWorld.OwnWgpu(ClusterCullingBindGroupLayout.CreateBindGroup(
            frame.Device.GetWgpu<WGPUDevice>(), BindGroupLayout.GetWgpu<WGPUBindGroupLayout>(),
            clusters.ConfigBuffer.GetWgpu<WGPUBuffer>(), lights.ClusteredBuffer.GetWgpu<WGPUBuffer>(), lights.ClusteredCapacity,
            clusters.LightGridBuffer.GetWgpu<WGPUBuffer>(), clusters.LightGridSize,
            clusters.LightIndexListBuffer.GetWgpu<WGPUBuffer>(), clusters.LightIndexListCapacity));
